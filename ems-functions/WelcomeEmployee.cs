...
namespace ems_functions
{
    public class WelcomeEmployee
    {
        ...
        public Task Run(
            [ServiceBusTrigger("employee-onboarding", Connection = "ServiceBusConnection")] string message)
        {
            ...
            try
            {
                ...
                if (employee != null)
                {
                    ...
                    _logger.LogInformation($"Dept: {employee.Department}, Joined: {employee.DateOfJoining}, Salary: {employee.Salary}");

                    // TODO: Call EMS API or trigger workflows here
                }
                else
                {
                    _logger.LogWarning("Message could not be deserialized into EmployeeDto.");
                }
            }
